...
- class: ResourceRequirement
  coresMin: $([inputs.runtime_cpu, 1].filter(function (inner) { return inner != null
    })[0])
  outdirMin: |-
    $([inputs.runtime_disks, 20].filter(function (inner) { return inner != null })[0])
  ramMin: |-
    $(Math.round((953.674 * [inputs.runtime_memory, 4].filter(function (inner) { return inner != null })[0])))
...
- id: outputFilename
  ...
  inputBinding:
    ...
    position: 5
    valueFrom: |-
      $(inputs.sam.basename.replace(/.cram$/, "").replace(/.bam$/, "").replace(/.sam$/, "")).bam
- id: regions
  label: regions
  doc: |-
